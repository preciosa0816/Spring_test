...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$("button[data-oper='modify']").on("click", function(e) {
			operForm.attr("action", "/mypage/wishlistmodify").submit();
		});
		$("button[data-oper='cancle']").on("click", function(e) {
			operForm.attr("action", "/mypage/wishlistcancle").attr("method","post").submit();
		}); 
		$("button[data-oper='list']").on("click", function(e) {
			operForm.find("#wno").remove();
			<sec:authentication property="principal" var="pinfo" />
			operForm.append('<input type="hidden" name="user_id" value="<c:out value='${pinfo.username}'/>" />');
			operForm.attr("action", "/mypage/wishlist");
			operForm.submit();
		});
...
<div class="row">
	<div class="col-lg-12">
		<h1 class="page-header">나의 희망도서 신청현황</h1>
	</div>
	<!-- /.col-lg-12 -->
</div>
<!-- /.row -->
	...
	<div class="col-lg-12">
		<div class="panel panel-default">
			...
			<table id='app_table'>
				...
				<c:if test="${wishlist.progress eq '신청대기중'}">
				<button data-oper='modify' class="btn btn-default">
						수정</button>	
				<button data-oper='cancle' class="btn btn-danger">
						신청취소</button></c:if>
				<button data-oper='list' class="btn btn-info"
					onclick="location.href='/mypage/wishlist'">목록</button>

				<form id="operForm" action="/mypage/wishlistmodify" method="get">
